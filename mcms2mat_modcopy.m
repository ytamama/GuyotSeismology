function mcms2mat_modcopy(yyyy,mm,dd,HH,MM,SS,qp,pdf,of,xls,yls)
...
% pdf      Quick pdf print as we go along [default: 0 for no]
...
% TEST INPUTS %%%
%Campus blast hours on Princeton University Campus
%16-17 UTC before daylight saving
%15-16 UTC after daylight saving


% Default data directory with the YYYY/MM/DD directories, set your own 'MC'
dirx = '/data2/seismometer';                                         %where to find the miniseed file
%dirx2 = '';   %where to save the MAT file; directory name hidden for privacy


% My directory where response files are kept
defval('icor',1)
dirr = '/data2/seismometer/Responses';  
...
for index=1:length(HH)
  ...
  if qp==1
    figure(); %creates 1 figure every hour
    clf;  %clears figure every hour
  ...
  if qp==1
	...
	% For the blasting...
	defval('xls',[0 3600])   %default: 11:30 AM - 11:50 AM EST
        defval('yls',[-1*10^4 1*10^4])
	xlim(xls)
        ylim(yls)
  ...
  nix=1;
    ...
    fprintf('%s not created thus no further action\n',msx);
...
if qp*nix==1
    ...
    %save a png version of my seismogram
    %save x limits!!!
